/**
 * Batch runner — MACD + EMA with SL as a sweep dimension.
 *
 * SL: 10%-50% (5)
 * TP: 10%-50% (5)
 * TF: 1m, 5m (2)
 ...
 * Leverage: 20
 * = 100 combinations, 2 years, $1000 starting capital.
 *
 * Usage: bun run src/run/batch-macd-ema-v2.ts > results.csv 2> progress.log
 */
...
const SL_RETURNS = [-0.10, -0.20, -0.30, -0.40, -0.50];
...
const totalCombos = TIMEFRAMES.length * EMA_PERIODS.length * SL_RETURNS.length * TP_RETURNS.length;
console.error(`Running ${totalCombos} MACD+EMA backtests (${TIMEFRAMES.length} TF × ${EMA_PERIODS.length} EMA × ${SL_RETURNS.length} SL × ${TP_RETURNS.length} TP)`);
...
for (const tf of TIMEFRAMES) {
  for (const emaPeriod of EMA_PERIODS) {
    for (const slReturn of SL_RETURNS) {
      for (const tpReturn of TP_RETURNS) {
        const slLabel = (Math.abs(slReturn) * 100).toFixed(0);
        const tpLabel = (tpReturn * 100).toFixed(0);

        const agent = createMacdEmaAgent({
          name: `macd-ema${emaPeriod}-${tf}-sl${slLabel}-tp${tpLabel}`,
          timeframe: tf,
          emaPeriod,
          tpReturn,
          slReturn,
        ...
        console.log([
          LEVERAGE, tf, emaPeriod, slLabel, tpLabel,
          metrics.totalTrades,
        ...
        console.error(`  ${completed}/${totalCombos} (${rate.toFixed(1)}/s, ~${remaining.toFixed(0)}s): ema${emaPeriod} ${tf} sl${slLabel}% tp${tpLabel}% → $${endCapital.toFixed(2)}`);
      }
    }
  }
}
